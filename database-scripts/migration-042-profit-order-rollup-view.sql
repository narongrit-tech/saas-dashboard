-- ============================================
-- Migration 042: Profit Reports Order-Level Rollup View
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(SUM(cogs.cost_allocated), 0) as cogs,
    SUM(s.order_amount) - COALESCE(SUM(ads.spend), 0) - COALESCE(SUM(cogs.cost_allocated), 0) as net_profit,
    p_user_id
  ...
  SELECT
    ...
    SUM(s.order_amount) - COALESCE(SUM(oa.commission_amt), 0) - COALESCE(SUM(cogs.cost_allocated), 0) as profit,
    p_user_id
  FROM sales_orders_order_rollup s
