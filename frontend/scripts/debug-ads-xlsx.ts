...
if (!filePath) {
  console.error('Usage: npx tsx scripts/debug-ads-xlsx.ts <path-to-xlsx>')
  process.exit(1)
}
...
const COLUMN_TOKENS = {
  date:     ['date','วันที่','วันเริ่มต้น','เวลาเริ่มต้น','start date','start time','日期'],
  campaign: ['campaign','แคมเปญ','ชื่อแคมเปญ','ad name','creative','campaign name','活动'],
  cost:     ['cost','spend','ค่าใช้จ่าย','ad spend','total cost','费用'],
  gmv:      ['gmv','revenue','รายได้','มูลค่ายอดขาย','conversion value','total value','收入'],
  orders:   ['order','orders','คำสั่งซื้อ','จำนวนคำสั่งซื้อ','conversion','conversions','purchase','purchases','订单'],
...
for (const sheetName of workbook!.SheetNames) {
  ...
  // Aggregate for all rows + filter to 2026-02-01
  ...
  for (const row of rows!) {
    ...
    const rawDate = mapping.date ? parseDateRaw(row[mapping.date]) : null
  ...
  console.log()
  console.log(`  📈 Aggregates — filtered to ${TARGET_DATE} (${rowsFiltered} rows):`)
