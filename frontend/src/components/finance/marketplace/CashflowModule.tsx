...
} from '@/app/(dashboard)/finance/marketplace-wallets/cashflow-api-actions';
import type {
  CashflowSummary,
...
interface CashflowModuleProps {
  ...
  showMarketplaceSelector?: boolean;
}
...
export function CashflowModule({ marketplace, showMarketplaceSelector = true }: CashflowModuleProps) {
  ...
  const fetchDailySummary = useCallback(async () => {
    ...
    try {
      ...
      const result = await getDailyCashflowSummary(dateRange.startDate, dateRange.endDate, dailyPage, 14);
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className="text-muted-foreground">เงินเข้า–ออกจากแพลตฟอร์มขาย (TikTok Shop, Shopee, Lazada)</p>
        </div>
      ...
      {showMarketplaceSelector && (
        <div className="flex items-center gap-3">
          ...
          <Select disabled value={marketplace}>
            <SelectTrigger className="w-48">
              <SelectValue placeholder="Select marketplace" />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="tiktok-shop">{MARKETPLACE_LABELS['tiktok-shop']}</SelectItem>
              <SelectItem value="shopee">{MARKETPLACE_LABELS.shopee}</SelectItem>
              <SelectItem value="lazada">{MARKETPLACE_LABELS.lazada}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      )}
      ...
      <Alert className="border-blue-200 bg-blue-50">
        ...
        <AlertDescription className="text-sm text-gray-700">
          <strong>Marketplace Wallets</strong> คือเงินที่แพลตฟอร์มขายถือไว้ชั่วคราว เช่น TikTok Shop ก่อนโอนเข้าบัญชีบริษัท
          หน้านี้ยังไม่ใช่ Cashflow ทั้งบริษัท และยังไม่รวมค่าใช้จ่ายหรือเงินสดในบัญชีบริษัท
        </AlertDescription>
      </Alert>
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              {gap >= 0 ? <CheckCircle2 className="h-4 w-4 text-green-600" /> : <XCircle className="h-4 w-4 text-red-600" />}
            </CardHeader>
            <CardContent>
              <div className={`text-2xl font-bold ${gap >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                ฿{formatCurrency(Math.abs(gap))}
              </div>
              <p className="text-xs text-muted-foreground">{gap >= 0 ? 'Actual > Forecast' : 'Actual < Forecast'}</p>
            </CardContent>
          </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">แสดงตามวันเงินเข้าจริง (เวลาประเทศไทย – Asia/Bangkok)</p>
        </CardHeader>
        <CardContent>
          {dailySummaryError && (
            ...
            <>
                    ...
                    {dailySummary.rows.map((row) => {
                      ...
                      return (
                        <tr key={row.date} className="hover:bg-gray-50">
                          ...
                          <td className="px-4 py-3 text-sm text-right">฿{formatCurrency(row.forecast_sum)}</td>
                          <td className="px-4 py-3 text-sm text-right font-semibold">฿{formatCurrency(row.actual_sum)}</td>
                          <td className={`px-4 py-3 text-sm text-right font-semibold ${row.gap >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              ...
              <div className="flex items-center justify-between mt-4">
                <div className="text-sm text-muted-foreground">
                  Showing {(dailyPage - 1) * 14 + 1}-{Math.min(dailyPage * 14, dailySummary.pagination.totalCount)} of{' '}
                  {dailySummary.pagination.totalCount} days
                </div>
                <div className="flex items-center gap-2">
                  <Button variant="outline" size="sm" onClick={() => setDailyPage(dailyPage - 1)} disabled={dailyPage === 1}>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-muted-foreground">Click a tab to view raw transaction rows (lazy loaded)</p>
        </CardHeader>
        <CardContent>
          <Tabs value={activeTab || ''} onValueChange={(tab) => { setActiveTab(tab as TransactionType); setTransactionsPage(1); }}>
            <TabsList>
                        ...
                        <tbody className="divide-y divide-gray-200">
                          {transactions.rows.map((row: TransactionRow) => (
                            <tr key={row.id} className="hover:bg-gray-50">
                              ...
                              <td className="px-4 py-3 text-sm text-right font-semibold">฿{formatCurrency(row.amount)}</td>
      ...
      <ImportOnholdDialog open={importOnholdDialogOpen} onOpenChange={setImportOnholdDialogOpen} onSuccess={handleImportSuccess} />
      <ImportIncomeDialog open={importIncomeDialogOpen} onOpenChange={setImportIncomeDialogOpen} onSuccess={handleImportSuccess} />
    </div>
  );
}
