...
import { previewStudioAnalyticsFile } from '@/lib/content-ops/tiktok-studio-analytics-import'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const preview = await previewStudioAnalyticsFile(buffer, file.name, user.id)
    return NextResponse.json({ ok: preview.ok, preview }, { status: 200 })
  } catch (err) {
