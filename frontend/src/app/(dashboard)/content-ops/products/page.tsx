...
import { ProductFullTable } from '@/components/content-ops/full-table'
import { getProductTrends } from '../actions'
...
export default async function ProductsPage({
  ...
  const { top, all, error } = await getProductTrends(from, to, 50)
    ...
    <div className="space-y-8 max-w-7xl">
      ...
      <section>
        <div className="flex items-baseline gap-2 mb-3">
          <h2 className="text-base font-semibold">Top {Math.min(top.length, 50)} Products</h2>
        ...
        {top.length === 0 ? (
          ...
          <div className="border rounded-lg overflow-hidden">
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground truncate">
                      {p.topShopName ?? `${p.shopCount} shops`}
                    </p>
                  </div>

                  {/* Sparkline */}
                  <div className="flex justify-end">
                    <Sparkline data={p.dailyCounts} width={64} height={22} />
                  </div>

                  {/* Order items */}
                  <div className="text-right">
                    <p className="text-sm font-semibold tabular-nums">
                      {p.orderItems.toLocaleString()}
                    </p>
                  </div>

                  {/* % change */}
                  <div className="text-right">
                    <ChangeBadge changePercent={p.changePercent} isNew={p.isNew} />
                  </div>
                </Link>
              ))}
            </div>
            ...
            <div className="border-t px-4 py-2 bg-muted/20 text-xs text-muted-foreground">
              {top.length} products shown · sorted by order items (current period)
            </div>
          </div>
        )}
      </section>
