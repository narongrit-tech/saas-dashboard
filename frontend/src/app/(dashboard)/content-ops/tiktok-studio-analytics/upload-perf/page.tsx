...
async function previewFile(file: File): Promise<PreviewSummary> {
  ...
  const res = await fetch('/api/content-ops/tiktok-video-performance/preview', { method: 'POST', body: formData })
  ...
  return {
    ...
    dateRangeRaw: p.dateRangeRaw ?? null,
    sheetName: p.sheetName ?? null,
    duplicateVideoIdCount: p.duplicateVideoIdCount ?? 0,
  }
}
...
async function importFile(file: File): Promise<ImportResult> {
  const formData = new FormData()
  formData.append('file', file)
  const res = await fetch('/api/content-ops/tiktok-video-performance/import', { method: 'POST', body: formData })
  const json = await res.json()
  if (!res.ok || json.ok === false) {
    const msg = json.error?.message ?? `Import failed (HTTP ${res.status})`
    throw new Error(msg)
  }
  const r = json.result
...
export default function PerfStatsUploadPage() {
  ...
  return (
    <div className="max-w-2xl space-y-6">
      ...
      <div>
        <h1 className="text-xl font-semibold">Import TikTok Perf Stats</h1>
        <p className="text-sm text-muted-foreground mt-1">
          Upload ไฟล์ Excel จาก TikTok Creator Center
          (ไฟล์ <code className="text-xs bg-muted px-1 rounded">Creator-Video-Performance_*.xlsx</code>)
        </p>
      </div>
      ...
      >
        <input ref={inputRef} type="file" accept=".xlsx" multiple className="hidden" onChange={e => addFiles(e.target.files)} />
        <div className="space-y-2">
          <Upload className="h-8 w-8 text-muted-foreground mx-auto" />
          <p className="text-sm font-medium">Drop .xlsx ที่นี่ หรือคลิกเพื่อเลือกไฟล์</p>
          <p className="text-xs text-muted-foreground">
            ไฟล์อยู่ที่: <code className="bg-muted px-1 rounded">D:\AI_OS\projects\bot-runner\downloads\video-analysis\</code>
          </p>
        </div>
      </div>
          ...
          {queue.map(item => (
            <div key={item.id} className="rounded-lg border bg-card">
              <div className="flex items-center gap-3 px-4 py-3">
                <FileSpreadsheet className={cn('h-5 w-5 shrink-0',
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs">
                    ...
                    <div>
                      ...
                      <p className="font-semibold text-xs truncate">
                        {item.previewSummary.dateRangeRaw ?? '—'}
                      </p>
                    </div>
                  </div>
                  {item.previewSummary.duplicateVideoIdCount > 0 && (
                    <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1">
                      พบ Video ID ซ้ำ {item.previewSummary.duplicateVideoIdCount} รายการในไฟล์
                    </p>
                  )}
                  {item.previewSummary.isDuplicateFile && (
                    <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded px-2 py-1">
                      ไฟล์นี้ถูก import แล้ว (batch {item.previewSummary.existingBatchId?.slice(0, 8)}…)
                    </p>
                  )}
                </div>
              )}
          ...
          <>
            {doneCount > 0 && (
              <Button asChild size="sm" variant="outline">
                <Link href="/content-ops/tiktok-studio-analytics?view=perf">ดู Perf Stats</Link>
              </Button>
            )}
