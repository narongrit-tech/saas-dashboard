    ...
    if (!pass1Complete && bundleSkuArray.length > 0) {
      ...
      while (true) {
        ...
        const { data: windowOrders, error: p1Error } = await supabase
          ...
          .not('shipped_at', 'is', null)
          .or('status_group.eq.เสร็จสมบูรณ์,and(status_group.eq.จัดส่งแล้ว,marketplace.eq.tiktok_shop),status_group.eq.สำเร็จแล้ว,status_group.ilike.ผู้ซื้อได้รับสินค้าแล้ว*,status_group.eq.จัดส่งสำเร็จแล้ว')
          .gte('order_date', `${startDateISO}T00:00:00+07:00`)
          .lte('order_date', `${endDateISO}T23:59:59+07:00`)
...
export async function getInventoryAvailabilityMaps(): Promise<{
  ...
  try {
    ...
    // ============================================
    // Query: only unshipped orders (still in warehouse, not yet handed to courier)
    // Excludes shipped (จัดส่งแล้ว) — stock already left warehouse, handled by daily COGS
    // Excludes cancelled and completed statuses
    const { data: orders, error: ordersError } = await supabase
      .from('sales_orders')
      .select('seller_sku, quantity')
      .is('shipped_at', null)
      .not('status_group', 'in', '("ยกเลิกแล้ว","เสร็จสมบูรณ์","ยกเลิก","สำเร็จแล้ว","จัดส่งสำเร็จแล้ว")')
